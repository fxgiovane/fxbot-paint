(() => {
    ...
    const LANGS = {
        pt: {
            ...
            turboMode: 'Modo Turbo (Máx. Velocidade)'
        },
        en: {
            title: `FXBot ${VERSION}`,
            tab_control: 'Control',
    ...
    const state = {
        ...
        lang: (localStorage.getItem(`fxbot-lang:${location.host}`) || 'auto'),
        _resolvedLang: 'en',
        manualStart: {
            enabled: false,
            index: 0
    ...
    const tKey = () => LANGS[state._resolvedLang];
    const t = (id, params) => {
    ...
    function buildUI() {
        ...
        dom.fileInput.addEventListener('change', onFile);
        dom.resizeBtn.addEventListener('click', resizeImage);
        dom.posBtn.addEventListener('click', selectPosition);
    ...
    function cacheDomElements() {
        ...
        dom.uploadBtn = qs('#fx-upload', root);
        dom.resizeBtn = qs('#fx-resize', root);
        dom.posBtn = qs('#fx-pos', root);
        dom.previewBtn = qs('#fx-preview', root);
        dom.fileInput = qs('#fx-file', root);
    ...
    function applyStateToUI() {
        ...
        const orderEl = qs('#fx-order', root);
    ...
    function resizeImage() {
        ...
        tmp.width = state.imgWidth;
        tmp.height = state.imgHeight;
    ...
    function centerPosOnCanvas() {
        const rect = canvasRect();
        if (!rect || !state.imgData) return false;
        const tile = Math.max(1, state.pixelSize | 0);
        ...
        for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
                ...
                if (!best) {
                    ...
                    best = sel;
                    state.colorCache.set(`${r},${g},${b}`, best);
                }
